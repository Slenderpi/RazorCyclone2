...
public readonly partial struct FirstPersonCharacterAspect : IAspect, IKinematicCharacterProcessor<FirstPersonCharacterUpdateContext>
{
    ...
    private void HandleVelocityControl(ref FirstPersonCharacterUpdateContext context, ref KinematicCharacterUpdateContext baseContext)
    {
        ...
        if (characterBody.ParentEntity != Entity.Null) {
            //characterControl.AccelVector = math.rotate(characterBody.RotationFromParent, characterControl.AccelVector);
            characterBody.RelativeVelocity = math.rotate(characterBody.RotationFromParent, characterBody.RelativeVelocity);
		}

        // Apply movement forces
        if (characterControl.Vacuum || characterControl.Cannon) {
            //float3 targetVelocity = characterControl.AccelVector * characterComponent.GroundMaxSpeed;
            float addForce = (characterControl.Vacuum ? 10f : 0f) + (characterControl.Cannon ? 100f : 0f);
            float3 targetVelocity = characterBody.RelativeVelocity + characterControl.AccelVector * addForce;
            if (characterControl.Vacuum)
		...
		//if (characterBody.IsGrounded && (characterControl.Vacuum || characterControl.Cannon)) {
		//	// Move on ground
  ...
  //          //float3 targetVelocity = characterControl.AccelVector * characterComponent.GroundMaxSpeed;
		//	float3 targetVelocity = characterBody.RelativeVelocity;
  //          float3 orientVector = characterControl.OrientationInput.z;
  //          if (characterControl.Vacuum)
  //          CharacterControlUtilities.StandardGroundMove_Interpolated(ref characterBody.RelativeVelocity, targetVelocity, characterComponent.GroundedMovementSharpness, deltaTime, characterBody.GroundingUp, characterBody.GroundHit.Normal);
  ...
  //      {
  //          // Move in air
  //          float3 airAcceleration = characterControl.AccelVector * characterComponent.AirAcceleration;
